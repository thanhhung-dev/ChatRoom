...
class LoginViewController: UIViewController {
  ...
  private lazy var socialStack: UIStackView = {
    ...
    let googleBtn = createSocialButton(
  ...
  private func animateOrbs() {
    UIView.animate(withDuration: 8.0, delay: 0, options: [.repeat, .autoreverse, .curveEaseInOut]) {
      self.ambientOrb1.transform = CGAffineTransform(translationX: 40, y: 30).scaledBy(
        x: 1.1, y: 1.1)
      self.ambientOrb2.transform = CGAffineTransform(translationX: -30, y: -40).scaledBy(
        x: 1.2, y: 1.2)
    } completion: { _ in
    }
  }
