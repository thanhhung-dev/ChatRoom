...
class WelcomeViewController: UIViewController {
    ...
    private let titleLabel: UILabel = {
        ...
        let attributedString = NSMutableAttributedString(
            string: "Box",
            attributes: [.foregroundColor: UIColor.textDark, .font: UIFont.systemFont(ofSize: 34, weight: .heavy)]
        )
        attributedString.append(NSAttributedString(
            string: "Chat",
            attributes: [.foregroundColor: UIColor.appBlue, .font: UIFont.systemFont(ofSize: 34, weight: .heavy)]
        ))
    ...
    private lazy var socialStack: UIStackView = {
        ...
        let googleBtn = makeSocialButton(imageName: "G_logo", sfSymbol: nil, tint: UIColor(red: 0.85, green: 0.26, blue: 0.21, alpha: 1))
    ...
    private func setupLayout() {
        let components = [logoImageView, titleLabel, taglineLabel, actionButtonStack, dividerLabel, socialStack]
    ...
    private func setupFloatingBoxes() {
        ...
        let w = view.bounds.width > 0 ? view.bounds.width : 393
        let configs: [BoxConfig] = [
            BoxConfig(x: w - 80,  y: 60,  size: 70, alpha: 0.12, rotation:  20, duration: 5.0, delay: 0.0, moveX: -8,  moveY:  10),
            BoxConfig(x: -20,     y: 110, size: 55, alpha: 0.09, rotation: -15, duration: 6.0, delay: 0.4, moveX:  10, moveY:  -8),
            BoxConfig(x: w - 60,  y: 230, size: 45, alpha: 0.07, rotation:  30, duration: 7.0, delay: 0.8, moveX:  -6, moveY:  12),
            BoxConfig(x: 10,      y: 340, size: 40, alpha: 0.08, rotation: -25, duration: 5.5, delay: 0.6, moveX:   8, moveY: -10),
            BoxConfig(x: w - 50,  y: 440, size: 35, alpha: 0.06, rotation:  15, duration: 6.5, delay: 1.2, moveX: -10, moveY:   8)
        ]
        ...
        for config in configs {
            ...
            UIView.animate(withDuration: 0.8, delay: config.delay + 0.3, options: .curveEaseOut) {
    ...
    private func startFloatingAnimation(_ iv: UIImageView, moveX: CGFloat, moveY: CGFloat, duration: Double, baseRotation: CGFloat) {
    ...
    private func runEntranceAnimation() {
        let sequence: [(UIView, TimeInterval)] = [
            ...
            (titleLabel,         0.2),
            (taglineLabel,       0.3),
            (actionButtonStack,  0.4),
            (dividerLabel,       0.5),
            (socialStack,        0.6)
        ]

        for (view, delay) in sequence {
    ...
    private func startLogoPulse() {
        UIView.animate(withDuration: 2.5, delay: 1.0, options: [.repeat, .autoreverse, .curveEaseInOut]) {
