...
class SplashViewController: UIViewController {
  ...
  private let loadingBarGradient = CAGradientLayer()
  ...
  override func viewDidLayoutSubviews() {
    ...
    loadingBarGradient.frame = loadingBar.bounds
    updateGridPath()
  ...
  private func setupOrbs() {
    ...
    let configs = [
      OrbConfig(
        view: orb1, size: 260, x: -80, y: 60, r: 0.19, g: 0.47, b: 1.0, alpha: 0.45, delay: 0.0),
      OrbConfig(
        view: orb2, size: 200, x: 160, y: 220, r: 0.39, g: 0.78, b: 1.0, alpha: 0.35, delay: 1.2),
      OrbConfig(
        view: orb3, size: 180, x: 30, y: 460, r: 0.31, g: 0.63, b: 1.0, alpha: 0.30, delay: 2.4),
    ]
  ...
  private func setupLogoIcon() {
    ...
    logoIconView.clipsToBounds = false

    let iconGrad = CAGradientLayer()
    iconGrad.frame = logoIconView.bounds
    iconGrad.colors = [
      UIColor(red: 0.15, green: 0.50, blue: 1.0, alpha: 1).cgColor,
      UIColor(red: 0.02, green: 0.25, blue: 0.80, alpha: 1).cgColor,
    ]
    iconGrad.startPoint = CGPoint(x: 0, y: 0)
    iconGrad.endPoint = CGPoint(x: 1, y: 1)
    iconGrad.cornerRadius = 20
    iconGrad.cornerCurve = .continuous
    logoIconView.layer.addSublayer(iconGrad)

    if let validImage = UIImage(named: "IconApp (1)") ?? UIImage(named: "IconLoad") {
      logoImageView.image = validImage
    }
    logoImageView.contentMode = .scaleAspectFit
    logoImageView.frame = CGRect(x: 12, y: 12, width: 44, height: 44)
    logoIconView.addSubview(logoImageView)

    logoIconView.layer.shadowColor = UIColor(red: 0.19, green: 0.47, blue: 1.0, alpha: 0.8).cgColor
    logoIconView.layer.shadowRadius = 24
    logoIconView.layer.shadowOpacity = 0.7
    logoIconView.layer.shadowOffset = CGSize(width: 0, height: 12)
  ...
  private func setupLoadingBar() {
    loadingTrack.backgroundColor = UIColor(white: 1, alpha: 0.05)
    loadingTrack.layer.cornerRadius = 1.5
    loadingTrack.clipsToBounds = true
    loadingTrack.alpha = 0

    loadingBar.layer.cornerRadius = 1.5
    loadingBar.clipsToBounds = true

    loadingBarGradient.colors = [
      UIColor(red: 0.19, green: 0.47, blue: 1.0, alpha: 1).cgColor,
      UIColor(red: 0.38, green: 0.67, blue: 1.0, alpha: 1).cgColor,
    ]
    loadingBarGradient.startPoint = CGPoint(x: 0, y: 0.5)
    loadingBarGradient.endPoint = CGPoint(x: 1, y: 0.5)
    loadingBar.layer.addSublayer(loadingBarGradient)
    ...
    NSLayoutConstraint.activate([
      ...
      loadingTrack.bottomAnchor.constraint(
        equalTo: view.safeAreaLayoutGuide.bottomAnchor, constant: -50),
      loadingTrack.widthAnchor.constraint(equalToConstant: 140),
      loadingTrack.heightAnchor.constraint(equalToConstant: 3),
  ...
  private func runAnimationSequence() {
    ...
    DispatchQueue.main.asyncAfter(deadline: .now() + 1.8) { [weak self] in
      guard let self else { return }
      self.loadingBarWidth?.constant = 140
